...
def dominant_color_palette(colours,thresh,counts):
  ...
  for ind in range(len(colours)):
    ...
    bdis1=euclid_distance(pixel_array,Black1)
    gdis1=euclid_distance(pixel_array,Grey1)
    sdis1=euclid_distance(pixel_array,Sky1)


    dist_list=[wdis1,bdis1,gdis1,sdis1]
  ...
  #index style BRYOGBr
  num=wcounts1+bcounts1+gcounts1 #put rare counts only
  deno=sum([wcounts1+bcounts1+gcounts1+scounts1])
  
  if num/deno>thresh:# check condition if max pixels values belongs to rare visual words or not
    final_count_list=[wcounts1,bcounts1,gcounts1,0] # make nonrare visual counts as 0
  else:
    final_count_list=[wcounts1,bcounts1,gcounts1,scounts1]
...
def main():
    ...
    Black1=np.asarray([0,0,0])
    Grey1=np.asarray([187, 197 ,199])
    Sky1=np.asarray([135, 206, 235])
    ...
    for image_index in range(len(pathname_list)):
      ...
      patch_to_id=[]
      img=image_resize(pathname_list,image_index) #resizing image 100x100x3
    ...
    words=[White1,Black1,Grey1,Sky1]
    with open(input_path+'vocab','wb')as l3:
      pickle.dump(words,l3)

    voc_index_list=['White1','Black1','Grey1','Sky1']
